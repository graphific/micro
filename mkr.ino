...
void setup() {
  ...
  if(measureVolt) {
    ...
    //TODO test of no value then skip
    // take a number of analog samples and add them up
      while (sample_count < NUM_SAMPLES) {
          float vv = analogRead(voltagePin);
          sum += vv;
          Serial.println (vv);
      ...
      temp = ((float)sum / (float)NUM_SAMPLES) / 1024.0;
  ...
  while (!connected) {
    ...
    if ((gsmAccess.begin() == GSM_READY) &&
      ...
      Serial1.println("[r"+String(epoch)+"]");
...
void readDHT() {
  ...
  if (isnan(h2) || isnan(t2) || isnan(f2)) {
    ...
    humidity = (humidity +h2)/2;
    bat_temp = (bat_temp+t2)/2;
  }
  // Compute heat index in Fahrenheit (the default)
